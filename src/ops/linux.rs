...
impl super::SystemProcess for LinProcess {
    ...
    fn open(&mut self) -> Result<(),String> {
        match self.state {
            State::Created => {
                let memfile = File::options().read(true).write(true).open("/proc/{}/mem").map_err(|err| err.to_string())?;
    ...
    fn read_memory(&mut self, location: u64, dt: &impl DataType) -> Result<Vec<u8>, String> {
        let mut memfile = self.state.memfile().ok_or("Process is closed or not yet opened.")?;
        ...
        memfile.read_exact(&mut read_buffer).map_err(|err| err.to_string());
    ...
    fn write_memory(&mut self, location: u64, what: Vec<u8>) -> Result<(), String> {
        ...
        let bytes_written = memfile.get_mut_ref().write(&what).map_err(|err| err.to_string())?;
